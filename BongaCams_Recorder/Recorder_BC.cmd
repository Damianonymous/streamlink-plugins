...
ECHO.
SET n=0
FOR /F "tokens=*" %%A IN (%~dp1BC_Model.txt) DO (
SET /A n=n+1
SET _fav!n!=%%A
ECHO !n! %%A
)
...
SET FILENAME=%MODEL%_BC_%NOW%.mp4
...
ECHO.
TITLE %UMODEL%
"%~dp1Streamlink_Portable\Streamlink.exe" "https://bongacams.com/%MODEL%" 720p_alt,720p,best -o "%OUT_DIR%%FILENAME%"
TITLE %MODEL%
